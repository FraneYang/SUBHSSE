...
Public Class 数据监控
    ...
    Private Sub btnMonitor_Click(sender As Object, e As EventArgs) Handles btnMonitor.Click
        ...
        mIO.Command(GetConnectInfo(), "BeginWatch_Bro") '广播式
    End Sub
    ...
    Private Sub btnMonitorClose_Click(sender As Object, e As EventArgs) Handles btnMonitorClose.Click
        ...
        mIO.Command(GetConnectInfo(), "CloseWatch_Bro") '广播式
    End Sub
...
#Region "事件"
    ...
    Private Sub _PasswordError(oInfo As ConnectInfo, iCommandCode As Integer, oValue As Object)
        Select Case iCommandCode
            ...
            Case FCardCDrive.ConnectMain.eCommandCode.cmdBeginWatch
                MessageBox.Show("【打开监控】由于通讯密码错误，命令处理失败！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Case FCardCDrive.ConnectMain.eCommandCode.cmdCloseWatch
    ...
    Private Sub _WatchEvent(oInfo As ConnectInfo, iRecordCode As Integer, bData() As Byte)
        ...
        Select Case iRecordCode
            ...
            Case Else
                ...
                '常规记录
                If mIO.WatchRecordDecompile(ConnectInfo.e_EquptType.FC8900, iRecordCode, bData, oRecord) Then
                    Dim sDate As String = "时间异常"
                    ...
                    Me.Invoke(Sub() txtEvent.Text += $"事件：{mRecordTypeName（oRecord.RecordType）}{vbTab}操作：{mEvnetCodeName（oRecord.EventCode）}{ vbNewLine}")
                End If
        End Select
...
#End Region
    ...
    Private mRecordTypeName(10) As String '记录类型 含义数组集合
    Private mEvnetCodeName(300) As String '操作事件类型含义 含义数组集合
    ...
    Private Sub Init()
        '
        mRecordTypeName(Record.eRecordType.eCardRecord) = "读卡事件" '
        mRecordTypeName(Record.eRecordType.eButtonRecord) = "按钮记录"
        mRecordTypeName(Record.eRecordType.eDoorStateRecord) = "门磁记录"
        mRecordTypeName(Record.eRecordType.eSoftwareRecord) = "软件记录"
        mRecordTypeName(Record.eRecordType.eAlarmRecord) = "报警记录"
        mRecordTypeName(Record.eRecordType.eSystemRecord) = "系统记录"

        '读卡记录读卡记录
        mEvnetCodeName(Record.eRecordEvent.eCardOpen) = "合法开门"
        mEvnetCodeName(Record.eRecordEvent.ePasswordOpen) = "密码开门"
        mEvnetCodeName(Record.eRecordEvent.eCardAndPasswordOpen) = "卡加密码"
        mEvnetCodeName(Record.eRecordEvent.eInputCardOpen) = "手动输入卡加密码"
        mEvnetCodeName(Record.eRecordEvent.eFirstCardOpen) = "首卡开门"
        mEvnetCodeName(Record.eRecordEvent.eCardHoldOpen) = "门常开-读卡"
        mEvnetCodeName(Record.eRecordEvent.eMuchCardOpen) = "多卡--门未开， 等待继续读卡"
        mEvnetCodeName(Record.eRecordEvent.eCardRepeat) = "重复读卡"
        mEvnetCodeName(Record.eRecordEvent.eCardExpire) = "有效期过期"
        mEvnetCodeName(Record.eRecordEvent.eCardTimeGroupExpire) = "开门时段过期"
        mEvnetCodeName(Record.eRecordEvent.eHolidayInvalid) = "节假日无效"
        mEvnetCodeName(Record.eRecordEvent.eCardInvalid) = "非法卡"
        mEvnetCodeName(Record.eRecordEvent.ePatrolCard) = "巡更卡"
        mEvnetCodeName(Record.eRecordEvent.eProbeLocked) = "探测锁定"
        mEvnetCodeName(Record.eRecordEvent.eCardOpenCountInvalid) = "有效次数已用尽"
        mEvnetCodeName(Record.eRecordEvent.eSlipInto) = "防潜回"
        mEvnetCodeName(Record.eRecordEvent.ePasswordError) = "密码错误"
        mEvnetCodeName(Record.eRecordEvent.eCardPasswordError) = "密码加卡-密码错误"
        mEvnetCodeName(Record.eRecordEvent.eCardLocked) = "锁定时禁止读卡"
        mEvnetCodeName(Record.eRecordEvent.eLockedPassword) = "锁定时禁止密码"
        mEvnetCodeName(Record.eRecordEvent.eFirstCardInvalid) = "首卡未开门"
        mEvnetCodeName(Record.eRecordEvent.eCardLose) = "挂失卡"
        mEvnetCodeName(Record.eRecordEvent.eCardBlacklist) = "黑名单卡"
        ...
        mEvnetCodeName(Record.eRecordEvent.eAreaCheckRepeatInoutOffline) = "区域防潜回-拒绝进入(与服务器失去连接)"
        mEvnetCodeName(Record.eRecordEvent.eArealinkageLockOffline) = "区域互锁-拒绝进入(与服务器失去连接)"
        mEvnetCodeName(Record.eRecordEvent.eAreaCheckRepeatInout) = "区域防潜回--拒绝开门"
