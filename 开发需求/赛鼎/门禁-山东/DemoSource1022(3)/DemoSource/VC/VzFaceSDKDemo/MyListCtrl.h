...
// Operations
	...
	void createItemButton(int nItem, int nSubItem, HWND hMain, LPCTSTR lpszCaption,int type, int nIDC);
	//释放创建的Button
...
public:
	...
	void createItemButton(int nItem, int nSubItem, HWND hMain, LPCTSTR lpszCaption, void * pData, int nID1 = 0, int nID2 = 0);
	//释放创建的Button
    void release();
	button_map m_mButton;
	button_map m_mSubButton;
	button_map m_mButtonDel;
	UINT m_uID;
	CFont font;    //按钮上面的字体
...
private:
	...
	void MoveButton(CRect rc, CButtonEx* btn);
...
public:
	void SetImagePath(int nItem, CString filePath);
	void CMyListCtrl::ShowButton(int nItem);
	std::map<int, CString> m_mapImg;
