...
SELECT CheckSpecialDetail.CheckSpecialDetailId
	...
	,(CASE WHEN CheckSpecial.States='0' OR CheckSpecial.States IS NULL THEN '待['+ISNULL(OperateUser.UserName,Users.UserName)+']提交' WHEN CheckSpecial.States='2' THEN '审核/审批完成' ELSE '待['+OperateUser.UserName+']办理' END) AS  FlowOperateName
...
UPDATE SafetyDataE_SafetyDataEPlan SET ReminderDate= DATEADD(DAY,-7,CheckDate)
GO

UPDATE SafetyData_SafetyDataPlan SET ReminderDate=  DATEADD(DAY,-7,CheckDate)
GO