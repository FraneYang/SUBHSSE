...
namespace BLL
{
    ...
    public static class UnitService
    {
        ...
        public static List<Model.Base_Unit> GetSubUnitList(string unitId)
        {
            ...
            var unitIdList = (from x in Funs.DB.Base_Unit  where x.SupUnitId == unitId
                             select x.UnitId).ToList();

            if (unitIdList.Count() > 0)
            {
                unitList = (from x in Funs.DB.Base_Unit where unitIdList.Contains(x.UnitId) || x.UnitId == unitId
                            orderby x.UnitCode descending
                            select x).ToList();
            }
            else
            {
                unitList = (from x in Funs.DB.Base_Unit where x.UnitId == unitId                         
                            select x).ToList();
            }
            return unitList;
        }
