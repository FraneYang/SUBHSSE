...
namespace BLL
{
    public static class APIBaseInfoService
    {
        ...
        public static List<Model.BaseInfoItem> getSpeciaCheckTypes(string type)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getDataLists = (from x in db.HSSE_Check_CheckItemSet
        ...
        public static List<Model.BaseInfoItem> getSpeciaCheckItemDetails(string type)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getDataLists = (from x in db.HSSE_Check_CheckItemDetail
        ...
        public static List<Model.BaseInfoItem> getProjectCheckItemSet(string projectId, string checkType, string supCheckItem)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getDataLists = (from x in db.Check_ProjectCheckItemSet
        ...
        public static List<Model.BaseInfoItem> getProjectCheckItemDetails(string type)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getDataLists = (from x in db.Check_ProjectCheckItemDetail
                                    where x.CheckItemSetId == type
                                    orderby x.SortIndex
                                    select new Model.BaseInfoItem { BaseInfoId = x.CheckItemDetailId, BaseInfoCode = x.SortIndex.ToString(), BaseInfoName = x.CheckContent }).ToList();
                return getDataLists;
        ...
        public static List<Model.BaseInfoItem> getProjectWorkArea(string projectId)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getDataLists = (from x in db.ProjectData_WorkArea
        ...
        /// </summary>
        /// <returns></returns>
        public static List<Model.BaseInfoItem> getProjectWorkPost(string projectId, string strParam)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getIdLists = db.SitePerson_Person.Where(x => (x.ProjectId == projectId || (projectId == null && x.ProjectId == null))).Select(x => x.WorkPostId).Distinct();

                var getDataLists = (from x in db.Base_WorkPost
                                    where getIdLists.Contains(x.WorkPostId) && (strParam == null || x.WorkPostName.Contains(strParam))
                                    orderby x.WorkPostName
                                    select new Model.BaseInfoItem { BaseInfoId = x.WorkPostId, BaseInfoCode = x.WorkPostCode, BaseInfoName = x.WorkPostName }).ToList();
                return getDataLists;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static Model.BaseInfoItem getAppPage(int setId)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getDataLists = (from x in db.Sys_Set
                                    where x.SetId == setId
                                    select new Model.BaseInfoItem { BaseInfoId = x.SetId.ToString(), BaseInfoCode = x.SetId.ToString(), BaseInfoName = System.Web.HttpUtility.HtmlDecode(x.SetValue) }).FirstOrDefault();
                return getDataLists;
            }
        }
        #endregion
