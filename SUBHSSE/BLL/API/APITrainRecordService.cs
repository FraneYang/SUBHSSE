        ...
        public static List<Model.TrainRecordItem> getTrainRecordListByProjectIdTrainTypeIdTrainStates(string projectId, string trainTypeId, string trainStates)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                var getDataLists = (from x in db.EduTrain_TrainRecord
                                    where (x.ProjectId == projectId || (projectId == null && x.ProjectId == null)) && x.TrainTypeId == trainTypeId
        ...
        public static void InsertTrainRecord(Model.Training_TestPlan getTestPlan)
        {
            using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
            {
                ...
                if (getTrainingPlan != null && getTrainRecord == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(getTrainingPlan.TrainTypeId))
                    {
                        newTrainRecord.TrainTypeId = getTrainingPlan.TrainTypeId;
                    }
                    else
                    {
                        newTrainRecord.TrainTypeId = getTestPlan.TrainTypeId;
                    }

                    if (!string.IsNullOrEmpty(getTrainingPlan.ProjectId))
                    {
                        newTrainRecord.ProjectId = getTrainingPlan.ProjectId;
                    }
