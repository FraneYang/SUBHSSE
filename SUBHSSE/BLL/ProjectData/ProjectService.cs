namespace BLL
{
    ...
    public static class ProjectService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetProjectNamesByProjectIds(string projectIds)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<Model.Base_Project> GetProjectWorkByUnitIdList(string unitId)
        {
            var list = (from x in Funs.DB.Base_Project
                        where x.UnitId == unitId && (x.ProjectState == null || x.ProjectState == BLL.Const.ProjectState_1)
                        orderby x.ProjectName 
                        select x).ToList();
            return list;
        }
        ...
        public static void InitProjectByUnitIdDropDownList(FineUIPro.DropDownList dropName,string unitId, bool isShowPlease)
        {
            dropName.DataValueField = "ProjectId";
            dropName.DataTextField = "ProjectName";
            dropName.DataSource = BLL.ProjectService.GetProjectWorkByUnitIdList(unitId);
            dropName.DataBind();
            if (isShowPlease)
            {
                Funs.FineUIPleaseSelect(dropName);
