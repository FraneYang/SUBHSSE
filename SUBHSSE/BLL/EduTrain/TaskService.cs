...
namespace BLL
{
    ...
    public static class TaskService
    {
        ...
        public static void DeleteTaskById(string taskId)
        {
            ...
            if (task != null)
            {
                var getTaskItemSchedule = from x in db.Training_TaskItemSchedule where x.TaskId == taskId select x;
                if (getTaskItemSchedule.Count() > 0)
                {
                    db.Training_TaskItemSchedule.DeleteAllOnSubmit(getTaskItemSchedule);
                    db.SubmitChanges();
                }
                var getTaskItem = from x in db.Training_TaskItem where x.TaskId == taskId select x;
                if (getTaskItem.Count() > 0)
                {
                    db.Training_TaskItem.DeleteAllOnSubmit(getTaskItem);
                    db.SubmitChanges();
                }
