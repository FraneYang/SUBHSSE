...
namespace BLL
{
    ...
    public static class TrainTypeService
    {
        ...
        /// <param name="trainTypeId"></param>
        /// <returns></returns>
        public static string GetTrainTypeNameById(string trainTypeId)
        {
            string name = string.Empty;
            var getType = Funs.DB.Base_TrainType.FirstOrDefault(e => e.TrainTypeId == trainTypeId);
            if (getType != null)
            {
                name = getType.TrainTypeName;
            }
            return name;
        }
        /// <summary>
