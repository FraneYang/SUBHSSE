    ...
    public class PersonController : ApiController
    {
        ...
        public Model.ResponeData getPersonStatesCount(string projectId, string unitId, string states, string strUnitId, string strWorkPostId, string strParam)
        {
            ...
            try
            {
                using (Model.SUBHSSEDB db = new Model.SUBHSSEDB(Funs.ConnString))
                {
                    var getViews = from x in db.SitePerson_Person
                                   ...
                                   select x;
                    if ((unitId != Const.UnitId_7 && unitId != Const.UnitId_SEDIN) && !string.IsNullOrEmpty(unitId))
                    {
            ...
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-人员数量", "PersonController.getPersonStatesCount");
            }
            return responeData;
        }
        #endregion
        ...
        public Model.ResponeData getPersonListByProjectIdStates(string projectId, string unitId, string states, string strUnitId, string strWorkPostId, string strParam, int pageIndex)
        {
            ...
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-人员列表", "PersonController.getPersonListByProjectIdStates");
            }
            return responeData;
        }
        #endregion
        ...
        public Model.ResponeData getPersonTestRecoedByIdentityCard(string identityCard, string projectId = null)
        {
            ...
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-获取人员培训考试信息", "PersonController.getPersonTestRecoedByIdentityCard");
            }
        ...
        public Model.ResponeData getTrainingPersonListByTrainTypeId(string projectId, string unitIds, string workPostIds, string trainTypeId, int pageIndex)
        {
            ...
            try
            {
                var getDataList = APIPersonService.getTrainingPersonListByTrainTypeId(projectId, unitIds, workPostIds, trainTypeId, null).OrderBy(x => x.UnitName).ThenBy(x => x.PersonName).ToList();
                int pageCount = getDataList.Count;
                if (pageCount > 0 && pageIndex > 0)
                {
                    getDataList = getDataList.Skip(Funs.PageSize * (pageIndex - 1)).Take(Funs.PageSize).ToList(); ////200 ->Funs.PageSize
                }
                responeData.data = new { pageCount, getDataList };
            }
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-考试人员信息", "PersonController.getTrainingPersonListByTrainTypeId");
            }
            return responeData;
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public Model.ResponeData postTrainingPersonListByTrainTypeId([FromBody] Model.PersonItem person)
        {
            var responeData = new Model.ResponeData();
            try
            {
                var getDataList = APIPersonService.getTrainingPersonListByTrainTypeId(person).OrderBy(x => x.UnitName).ThenBy(x => x.PersonName).ToList(); 
                int pageCount = getDataList.Count;
                if (pageCount > 0 && person.PageIndex > 0)
                {
                    getDataList = getDataList.Skip(Funs.PageSize * (person.PageIndex - 1)).Take(Funs.PageSize).ToList(); ////200 ->Funs.PageSize
                }
                responeData.data = new { pageCount, getDataList };
            }
            catch (Exception ex)
        ...
        public Model.ResponeData SaveSitePerson([FromBody] Model.PersonItem person)
        {
            ...
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-保存项目人员信息", "PersonController.SaveSitePerson");
            }
