...
namespace WebAPI.Controllers
{
    ...
    public class ResourcesController : ApiController
    {
        ...
        public Model.ResponeData getCompanyTrainingListBySupTrainingId(string supTypeId, int pageIndex)
        {
            ...
            try
            {
                var getDataList = APIResourcesService.getCompanyTrainingListBySupTrainingId(supTypeId, null);
                int pageCount = getDataList.Count;
                if (pageCount > 0 && pageIndex > 0)
                {
                    getDataList = getDataList.Skip(Funs.PageSize * (pageIndex - 1)).Take(Funs.PageSize).ToList();
        ...
        /// <param name="pageIndex">分页</param>
        /// <returns></returns>
        public Model.ResponeData getCompanyTrainingListBySupTrainingId(string supTypeId, string projectId, int pageIndex)
        {
            var responeData = new Model.ResponeData();
            try
            {
                var getDataList = APIResourcesService.getCompanyTrainingListBySupTrainingId(supTypeId, projectId);
                int pageCount = getDataList.Count;
                if (pageCount > 0 && pageIndex > 0)
                {
                    getDataList = getDataList.Skip(Funs.PageSize * (pageIndex - 1)).Take(Funs.PageSize).ToList();
        ...
        /// <param name="pageIndex">分页</param>
        /// <returns></returns>
        public Model.ResponeData getTestTrainingListBySupTrainingId(string supTypeId, string projectId, int pageIndex)
        {
            var responeData = new Model.ResponeData();
            try
            {
        ...
        /// <summary>
        /// 根据类型ID获取考试试题类型下各题目数量
        /// </summary>
        /// <param name="trainingId"></param>
        /// <returns></returns>
        public Model.ResponeData getTestTrainingCountByTrainingId(string trainingId)
        {
            var responeData = new Model.ResponeData();
            try
            {
                responeData.data = APIResourcesService.getTestTrainingCountByTrainingId(trainingId);
            }
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-获取各题目数量", "ResourcesController.getTestTrainingCountByTrainingId");
            }
            return responeData;
        }
        #endregion
        ...
        public Model.ResponeData getTestTrainingItemListByTrainingId(string trainingId, int pageIndex)
        {
            ...
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-获取考试试题列表", "ResourcesController.getTestTrainingItemListByTrainingId");
            }
            return responeData;
        }
        #endregion
        ...
        public Model.ResponeData getTestTrainingItemByTrainingItemId(string trainingItemId)
        {
            ...
            catch (Exception ex)
            {
                responeData.code = 0;
                responeData.message = ex.Message;
                ErrLogInfo.WriteLog(ex, "WX接口-获取考试试题详细信息", "ResourcesController.getTestTrainingItemByTrainingItemId");
            }
