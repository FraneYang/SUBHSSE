...
namespace FineUIPro.Web.InformationProject
{
    public partial class AccidentCauseReportEdit : PageBase
    {
        ...
        {             
            ...
            //}
            /// 天数
            int days = dayReports.Count();

            int sumPersonTotal = Convert.ToInt32(getDetail.Sum(x=>x.RealPersonNum) ?? 0);
            int sumPersonWorkTimeTotal = Convert.ToInt32(getDetail.Sum(x=>x.PersonWorkTime) ?? 0);
            if (days > 0)
            {
                //平均工时总数
                this.txtAverageTotalHours.Text = Convert.ToInt32(sumPersonWorkTimeTotal / days).ToString();
                //人数
                this.txtAverageManHours.Text =Convert.ToInt32 (sumPersonTotal / days).ToString();
            }
            else
            {  //平均工时总数
                this.txtAverageTotalHours.Text ="0";
                //人数
                this.txtAverageManHours.Text = "0";
            }

            var getAccidentReports = from x in Funs.DB.Accident_AccidentReport
                                     where x.ProjectId == this.ProjectId && x.States == BLL.Const.State_2 && x.AccidentDate >= startTime && x.AccidentDate < endTime
                                     select x;
            if (getAccidentReports.Count() > 0)
            {
                //死亡事故
                var accidentReports1 = getAccidentReports.Where(x => x.AccidentTypeId == "1");
                ...
                //重伤事故
                var accidentReports2 = getAccidentReports.Where(x => x.AccidentTypeId == "2");
                ...
                //轻伤事故
                var accidentReports3 = getAccidentReports.Where(x => x.AccidentTypeId == "3");
                ...
                this.txtTotalLossMan.Text = (getAccidentReports.Sum(x => x.WorkingHoursLoss) ?? 0).ToString();
                ...
                decimal totalLoseHours = getAccidentReports.Sum(x => x.WorkingHoursLoss ?? 0);
                ...
                this.txtDirectLoss.Text = (getAccidentReports.Sum(x => x.EconomicLoss ?? 0) + accidentReportOthers.Sum(x => x.EconomicLoss ?? 0)).ToString();
                this.txtIndirectLosses.Text = (getAccidentReports.Sum(x => x.EconomicOtherLoss ?? 0) + accidentReportOthers.Sum(x => x.EconomicOtherLoss ?? 0)).ToString();
            ...
            {
                //死亡事故
                this.txtDeathAccident.Text ="0";
                this.txtDeathToll.Text = "0"; ;
                //重伤事故
                this.txtInjuredAccident.Text = "0"; 
                this.txtInjuredToll.Text = "0";
                //轻伤事故
                this.txtMinorWoundAccident.Text = "0";
                this.txtMinorWoundToll.Text = "0";
                //损失工时总数
                this.txtTotalLossMan.Text = "0"; ;
                this.txtLastMonthLossHoursTotal.Text = "0";
                //歇工总日数
                this.txtKnockOffTotal.Text = "0"; ;
                //经济损失
                this.txtDirectLoss.Text = "0"; 
                this.txtIndirectLosses.Text = "0";
                this.txtTotalLoss.Text = "0";
            }
            //无损失工时总数
