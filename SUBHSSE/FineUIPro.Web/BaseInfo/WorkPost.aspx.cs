...
namespace FineUIPro.Web.BaseInfo
{
    public partial class WorkPost : PageBase
    {
        ...
        {        
            var q = (from x in Funs.DB.View_Base_WorkPost
                    orderby x.PostType, x.WorkPostCode
                    select x).ToList();
            if (!string.IsNullOrEmpty(this.txtName.Text.Trim()))
            {
                q = q.Where(x => x.WorkPostName.Contains(this.txtName.Text.Trim())).ToList();
            }
        ...
        private List<Model.Base_WorkPost> GetPagedDataTable(int pageIndex, int pageSize)
        {
            ...
            List<Model.Base_WorkPost> source = q;
