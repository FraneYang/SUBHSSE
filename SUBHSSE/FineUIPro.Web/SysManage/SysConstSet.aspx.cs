...
namespace FineUIPro.Web.SysManage
{
    public partial class SysConstSet : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (thisUnit != null)
                {
                    ...
                    if (thisUnit.UnitId == Const.UnitId_SEDIN || CommonService.GetIsThisUnit(Const.UnitId_7) || thisUnit.UnitId == Const.UnitId_XJYJ)
                    {
                ...
                this.LoadTab3Data();
                ////移动端欢迎页
                var getSys_Set = Funs.DB.Sys_Set.FirstOrDefault(x => x.SetId == 1);
                if (getSys_Set != null)
                {
                    this.txtAppPage.Text = HttpUtility.HtmlDecode(getSys_Set.SetValue);
                }
            }
        }
        ...
        private void BindGrid()
        {
            ...
            if ((CommonService.GetIsThisUnit(Const.UnitId_SEDIN) || CommonService.GetIsThisUnit(Const.UnitId_7) || CommonService.GetIsThisUnit(Const.UnitId_XJYJ)) && LicensePublicService.lisenWorkList.Contains(this.drpMenu.Value))
            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btnAppPage_Click(object sender, EventArgs e)
        {
            var getSys_Set = Funs.DB.Sys_Set.FirstOrDefault(x => x.SetId == 1);
            if (getSys_Set != null)
            {
                getSys_Set.SetValue = HttpUtility.HtmlEncode(this.txtAppPage.Text);
            }
            else
            {
                Model.Sys_Set newSet = new Model.Sys_Set
                {
                    SetId = 1,
                    SetName = "移动端欢迎页",
                    SetValue = HttpUtility.HtmlEncode(this.txtAppPage.Text),
                };
