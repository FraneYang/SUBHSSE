...
namespace FineUIPro.Web.EduTrain
{
    public partial class TestPlan : PageBase
    {
        ...
        {
            string strSql = @"SELECT TestPlan.TestPlanId,TestPlan.PlanName,TestPlan.PlanCode,TestPlan.PlanManId,PlanMan.UserName AS PlanManName,TestPlan.PlanDate,TestPlan.TestStartTime,TestPlan.TestEndTime,TestPlan.Duration,TestPlan.TotalScore,TestPlan.QuestionCount,TestPlan.TestPalce,TestPlan.UnitIds,TestPlan.UnitNames,TestPlan.DepartIds,TestPlan.DepartNames,TestPlan.QRCodeUrl"
                                + @" ,TestPlan.WorkPostIds,WorkPostNames=STUFF((SELECT ',' + WorkPostName FROM dbo.Base_WorkPost where PATINDEX('%,' + RTRIM(WorkPostId) + ',%',',' +TestPlan.WorkPostIds + ',')>0 FOR XML PATH('')), 1, 1,'')"
                                + @" ,TestPlan.States,(CASE WHEN TestPlan.States='1' THEN '已发布未考试'  WHEN TestPlan.States='2' THEN '考试中' WHEN TestPlan.States='3' THEN '考试结束' WHEN TestPlan.States='-1' THEN '已作废' ELSE '待提交' END) AS StatesName"
            ...
            {
                strSql += " AND TestPlan.ProjectId IS NULL";
            }
        ...
        private void EditData()
        {
            ...
            var getPlan = BLL.TestPlanService.GetTestPlanById(id);
            if (getPlan != null)
            {
                if (getPlan.States == BLL.Const.State_0 && !this.btnMenuEdit.Hidden && (this.CurrUser.UserId == getPlan.PlanManId || this.CurrUser.UserId == Const.sysglyId))
                {
        ...
        {
            string menuId = BLL.Const.ServerTestPlanMenuId;
            if (!string.IsNullOrEmpty(this.CurrUser.LoginProjectId))
            {
                menuId = BLL.Const.ProjectTestPlanMenuId;
            }
        ...
        private string judgementDelete(string id)
        {
            ...
            if (Funs.DB.Training_TestRecord.FirstOrDefault(x => x.TestPlanId == id && x.TestStartTime.HasValue) != null)
            {
