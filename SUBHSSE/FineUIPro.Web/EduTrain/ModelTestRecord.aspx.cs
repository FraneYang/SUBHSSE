    ...
    public partial class ModelTestRecord : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                BLL.ModelTestRecordService.UpdateTestEndTimeNull(null);
                // 绑定表格
                BindGrid();
            }
            else
        ...
        {
            string strSql = @"SELECT ModelTestRecord.ModelTestRecordId, ModelTestRecord.TestManId, ModelTestRecord.TestStartTime
                        , ModelTestRecord.TestEndTime, ModelTestRecord.TestScores,person.PersonName AS TestManName"
                         + @" ,TotalScore, Duration,QuestionCount"
            ...
            {
                strSql += " AND ModelTestRecord.ProjectId =@ProjectId";
                listStr.Add(new SqlParameter("@ProjectId", this.CurrUser.LoginProjectId));
            }
            else
            {
                strSql += " AND ModelTestRecord.ProjectId IS NULL";
            }
            //if (this.CurrUser.UserId != BLL.Const.sysglyId)
            //{
        ...
        private void DeleteData()
        {
            if (Grid1.SelectedRowIndexArray.Length > 0)
            {
                foreach (int rowIndex in Grid1.SelectedRowIndexArray)
                {
                    ...
                    if (getV != null && (getV.IdentityCard == this.CurrUser.IdentityCard  || this.CurrUser.UserId == Const.sysglyId))
                    {
