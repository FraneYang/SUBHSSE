...
namespace FineUIPro.Web.EduTrain
{
    public partial class CompanyTrainingItemSave : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                this.MenuId = BLL.Const.CompanyTrainingMenuId;
                if (!string.IsNullOrEmpty(this.CurrUser.LoginProjectId))
                {
                    this.MenuId = BLL.Const.ProjectCompanyTrainingMenuId;
                }
                this.GetButtonPower();
        ...
        protected void btnUploadResources_Click(object sender, EventArgs e)
        {
            ...
            {                
                PageContext.RegisterStartupScript(WindowAtt.GetShowReference(String.Format("../AttachFile/webuploader.aspx?toKeyId={0}&path=FileUpload/CompanyTraining&menuId={1}&type=-1", CompanyTrainingItemId, this.MenuId)));
            }
            else
            {
                if (string.IsNullOrEmpty(this.CompanyTrainingItemId))
                ...
                PageContext.RegisterStartupScript(WindowAtt.GetShowReference(String.Format("../AttachFile/webuploader.aspx?toKeyId={0}&path=FileUpload/CompanyTraining&menuId={1}", CompanyTrainingItemId, this.MenuId)));
            }
        }
        #endregion
