            ...
            if (pds != null && ir > 0)
            {
                ...
                for (int i = 0; i < ir; i++)
                {
                    Model.View_Training_TestTrainingItem newViewTrainingItem = new Model.View_Training_TestTrainingItem
                    {
                        ...
                        AbstractsEng = pds.Rows[i][6].ToString().Trim(),
                        AbstractsRussian = pds.Rows[i][7].ToString().Trim(),
                        AItem = pds.Rows[i][8].ToString().Trim(),
                        AItemEng = pds.Rows[i][9].ToString().Trim(),
                        AItemRussian= pds.Rows[i][10].ToString().Trim(),
                        BItem = pds.Rows[i][11].ToString().Trim(),
                        BItemEng = pds.Rows[i][12].ToString().Trim(),
                        BItemRussian = pds.Rows[i][13].ToString().Trim(),
                    };
                    ...
                    {
                        var training = trainings.FirstOrDefault(x => x.TrainingName == col1);
                        if (training != null)
                        {
                            newViewTrainingItem.TrainingId = training.TrainingId;
                            newViewTrainingItem.TrainingCode = training.TrainingCode;
                            newViewTrainingItem.TrainingName = training.TrainingName;
                        }
                        else
                        {
                            Model.Training_TestTraining newTraining = new Model.Training_TestTraining();
                            newViewTrainingItem.TrainingId = newTraining.TrainingId = SQLHelper.GetNewID();
                            newViewTrainingItem.TrainingName = newTraining.TrainingName = col1;
                            newTraining.TrainingCode = newViewTrainingItem.TrainingCode;
                            newTraining.ProjectId = this.CurrUser.LoginProjectId;
                    ...
                    else
                    {
                        ...
                        if (col3 == "单选题")
                        {
                            ...
                            newViewTrainingItem.CItem = pds.Rows[i][14].ToString().Trim();
                            newViewTrainingItem.CItemEng = pds.Rows[i][15].ToString().Trim();
                            newViewTrainingItem.CItemRussian = pds.Rows[i][16].ToString().Trim();
                            newViewTrainingItem.DItem = pds.Rows[i][17].ToString().Trim();
                            newViewTrainingItem.DItemEng = pds.Rows[i][18].ToString().Trim();
                            newViewTrainingItem.DItemRussian = pds.Rows[i][19].ToString().Trim();
                        ...
                        else if (col3 == "多选题")
                        {
                            ...
                            newViewTrainingItem.DItemEng = pds.Rows[i][18].ToString().Trim();
                            newViewTrainingItem.DItemRussian = pds.Rows[i][19].ToString().Trim();
                            newViewTrainingItem.EItem = pds.Rows[i][20].ToString().Trim();
                            newViewTrainingItem.EItemEng = pds.Rows[i][21].ToString().Trim();
                            newViewTrainingItem.EItemRussian = pds.Rows[i][22].ToString().Trim();
                            //if (string.IsNullOrEmpty(newViewTrainingItem.AItem) || string.IsNullOrEmpty(newViewTrainingItem.BItem) || string.IsNullOrEmpty(newViewTrainingItem.CItem) || string.IsNullOrEmpty(newViewTrainingItem.DItem))
                            //{
                    ...
                    {
                        int icount = 0;
                        List<string> selecItem = Funs.GetStrListByStr(col23, ',');
                        ...
                        if (icount == 0)
                        {
                            newViewTrainingItem.AnswerItems = col23.Replace("a", "A").Replace("b", "B").Replace("c", "C").Replace("d", "D").Replace("e", "E");
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (addTrainingItem == null)
                        {
                            ...
                            {
                                TrainingItemId = newViewTrainingItem.TrainingItemId = SQLHelper.GetNewID(),
                                ...
                                AItemEng = newViewTrainingItem.AItemEng,
                                BItemEng = newViewTrainingItem.BItemEng,
                                CItemEng = newViewTrainingItem.CItemEng,
                                DItemEng = newViewTrainingItem.DItemEng,
                                EItemEng = newViewTrainingItem.EItemEng,

                                AItemRussian = newViewTrainingItem.AItemRussian,
                                BItemRussian = newViewTrainingItem.BItemRussian,
                                CItemRussian = newViewTrainingItem.CItemRussian,
                                DItemRussian = newViewTrainingItem.DItemRussian,
                                EItemRussian = newViewTrainingItem.EItemRussian,
                            ...
                            if (!string.IsNullOrEmpty(newViewTrainingItem.AbstractsEng))
                            {
                                if (newViewTrainingItem.AbstractsEng.Contains("(") || newViewTrainingItem.AbstractsEng.Contains(")")
                                 || newViewTrainingItem.AbstractsEng.Contains("（") || newViewTrainingItem.AbstractsEng.Contains("）"))
                                {
                                    newTrainingItem.AbstractsEng = newViewTrainingItem.AbstractsEng;
                                }
                                else
                                {
                                    newTrainingItem.AbstractsEng = newViewTrainingItem.AbstractsEng + "( )";
                            ...
                            if (!string.IsNullOrEmpty(newViewTrainingItem.AbstractsRussian))
                            {
                                if (newViewTrainingItem.AbstractsRussian.Contains("(") || newViewTrainingItem.AbstractsRussian.Contains(")")
                                 || newViewTrainingItem.AbstractsRussian.Contains("（") || newViewTrainingItem.AbstractsRussian.Contains("）"))
                                {
                                    newTrainingItem.AbstractsRussian = newViewTrainingItem.AbstractsRussian;
                                }
                                else
                                {
                                    newTrainingItem.AbstractsRussian = newViewTrainingItem.AbstractsRussian + "( )";
